...
timeout -t 30 docker exec mf2c_micro_cau-client sh -c "${cau_registration}"
while [ $? -ne 0 ]
do
  timeout -t 60 docker exec mf2c_micro_cau-client sh -c "${cau_registration}"
done
...
docker run -d --network="host" \
        ...
        --label "PRODUCT=MF2C" \
        mf2c/discovery-microagent:4.10
...
docker run --rm -p 46300:46300 -p 46000:46000 --env-file env.list \
        ...
        --label "PRODUCT=MF2C" \
        mf2c/trunk:lifecycle-1.0.10-arm
